...
class AbstractFileCrawler(AbstractCrawler):
    def __init__(self, m3u8_uri, m3u8_file_path, customized_http_header=None, max_retry_times=3, num_concurrent=50, mp4_file_dir=None, mp4_file_name='m3u8-To-Mp4.mp4', tmpdir=None):
        super(AbstractFileCrawler, self).__init__(m3u8_uri, customized_http_header, max_retry_times, num_concurrent, mp4_file_dir, mp4_file_name, tmpdir)
    ...
    def _construct_key_segment_pairs_by_m3u8(self, m3u8_obj):
        ...
        for key in m3u8_obj.keys:
            if key:
                ...
                response_code, encryped_value = sync_http.retrieve_resource_from_url(self.best_addr_info, key.absolute_uri, customized_http_header=self.customized_http_header)
        ...
        if len(key_segments_pairs) == 0:
            raise M3u8PlaylistIsNoneException(name=self.m3u8_file_path, reason='M3u8 playlist is null!')
    ...
    def _create_tasks(self):
        ...
        if m3u8_obj.is_variant:
            raise M3u8FileIsVariantException(self.m3u8_file_path, 'M3u8 file is variant, and i do not support retrieve m3u8 in current mode!')
...
class AbstractUriCrawler(AbstractCrawler):
    def _request_m3u8_obj_from_url(self):
        try:
            response_code, m3u8_bytes = sync_http.retrieve_resource_from_url(self.best_addr_info, self.m3u8_uri, customized_http_header=self.customized_http_header)
            if response_code != 200:
